...
### 14. 기간 선택 UI는 달력이 아니라 기간 탭 구조로 간다

결정:

- UI는 `기간 유형 탭 -> 실제 기간 탭 -> 섹션 탭` 구조로 만든다.
- 1일은 날짜별, 1주는 월별 주차, 1개월은 월, 3개월은 분기, 6개월은 반기 버킷을 사용한다.

이유:

- 사용자는 1일/1주/1개월/3개월/6개월마다 보는 질문이 다르다.
- 달력 선택보다 "어떤 관점으로 볼 것인가"를 먼저 고르는 UI가 더 목적 중심이다.
- 기간별 데이터 항목이 달라질수록 이중 탭 구조가 더 자연스럽다.
