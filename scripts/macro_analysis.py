...
def load_fred_latest(root: Path, date_str: str) -> dict:
    ...
    for f in raw_dir.glob("*.json"):
        ...
        sid, val = meta.get("series_id"), meta.get("latest_value")
        if sid and val:
            try:
...
def load_ecos_latest(root: Path, date_str: str) -> dict:
    """Load BOK ECOS key indicators from raw files."""
    raw_dir = root / "data/raw" / date_str / "ecos_api"
    values = {}
    if not raw_dir.exists():
        return values
    for f in raw_dir.glob("*.json"):
        doc = json.loads(f.read_text())
        meta = doc.get("metadata", {})
        tag, val = meta.get("tag"), meta.get("value")
...
def analyze(root: Path, date_str: str) -> dict:
    ...
    if not market:
        return {"error": "market_data_latest.json 없음 — load_market_data.py 먼저 실행 필요"}
    ...
    nikkei, _,   _  = extract_series(a.get("Nikkei",  {}))
    dax,    _,   _  = extract_series(a.get("DAX",     {}))
    usdjpy, _,   _  = extract_series(a.get("USDJPY",  {}))
    eurusd, _,   _  = extract_series(a.get("EURUSD",  {}))

    fred = load_fred_latest(root, date_str)
    ecos = load_ecos_latest(root, date_str)

    # ── macro inputs (FRED 우선, 없으면 시장가/ECOS) ─────────────────────────
    ...
    # ── technical indicators (S&P500 기준) ────────────────────────────────────
    rsi_sp    = rsi(sp, 14)
